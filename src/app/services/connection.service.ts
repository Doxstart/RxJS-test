...
export class ConnectionService {
  ...
  constructor(private http: HttpClient) {
   ...
   getDittoWithPromise(): Promise<any>{
    // fetch(this.DITTO_URL).then(resp => resp.json())
    // .then(ditto => console.log('ditto fetch', ditto))
    // .catch(err => console.log(err));

    return fetch(this.DITTO_URL).then(resp => resp.json()); //devo aggiungere Promise<any> invece di void
   }

   getDittoWithObservable(): Observable<object>{
    // this.http.get(this.DITTO_URL)
    // .subscribe({
    //   next: ditto => console.log('ditto httpclient', ditto),
